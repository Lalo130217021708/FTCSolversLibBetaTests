...
import static com.qualcomm.hardware.lynx.commands.core.LynxSetMotorPIDFControlLoopCoefficientsCommand.InternalMotorControlAlgorithm.PIDF;
...
class CentripetalTuner extends OpMode {
    ...
    public void start() {
        follower.activateAllPIDFs();
        forwards = new Path(new BezierCurve(new Pose(0,0), new Pose(Math.abs(DISTANCE) + 0,0), new Pose(Math.abs(DISTANCE) + 0,DISTANCE + 0)));
        backwards = new Path(new BezierCurve(new Pose(Math.abs(DISTANCE) + 0,DISTANCE + 0), new Pose(Math.abs(DISTANCE) + 0,0), new Pose(0,0)));
